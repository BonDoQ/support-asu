...
@section('content')
  ...
  <!-- Events Panel -->
    <div class="col-sm-9">
        <form class="form-horizontal" method="POST" action="/cpanel/sponsors/{{$sponser->id}}" enctype="multipart/form-data">
          ...
          <div class="form-group">
              ...
              <div>
                ...
                @if($sponser->availibility == true) {{'checked'}}  @endif
          ...
          <div class="form-group">
              ...
              <div class="col-xs-10">
                ...
                <div class="col-sm-2 "><img class="img-responsive" src="{{asset($sponser->image_logo)}}"></div>
              </div>
          </div>
          <div class="form-group">
            <label for="inputlink" class="control-label col-xs-2">Website :</label>
              <div class="col-xs-10">
                <input type="text" class="form-control" id="inputlink" placeholder="Paste the Event Link " name="website" value="{{$sponser->website}}" required>
          ...
          <div class="form-group">
            <label for="short-des" class="control-label col-xs-2">Information :</label>
              <div class="col-xs-10">
                <textarea class="form-control" id="short-des" name="information" required>{{$sponser->information}}</textarea>
              </div>
          </div>
          <div class="form-group">
              ...
              <!-- <ul class="list-inline">
                ...
                <input name="{{$vent->id}}"
                  @foreach($ventsofsponser as $ventofsponser)
                    @if($ventofsponser->name == $vent->name) {{'checked'}} @endif
                  @endforeach
                type="checkbox">
                  {{$vent->name}}
                </li>
